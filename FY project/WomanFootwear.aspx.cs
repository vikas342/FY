...
{
    public partial class WomanFootwear : System.Web.UI.Page
    {
        ...
        protected void txtFilterGrid1Record_TextChanged(object sender, EventArgs e)
        {
            ...
            con.Open();
            string qr = "select A.*,B.*,C.Name ,A.PPrice-A.PSelPrice as DiscAmount,B.Name as ImageName, C.Name as BrandName from tblProducts A inner join tblBrands C on C.BrandID =A.PBrandID inner join tblCategory as t2 on t2.CatID=A.PCategoryID inner join tblSubCategory as t3 on t3.SubCatID= A.PSubCatID cross apply( select top 1 * from tblProductImages B where B.PID= A.PID order by B.PID desc )B where  t3.SubCatName='Womans footwear' Or t3.SubCatName='Womans Footwear' AND A.PName like '" + txtFilterGrid1Record.Text + "%' order by A.PID desc";
